# ===============================================================
# File    : u_model2B_augmented_regularized.py
# Author  : Udhaya Sankari
# Purpose : Model-2B – augmentation + dropout regularization (5 epochs)
#           Multimodal (Image + TF-IDF text). Uses your absolute paths.
#           Saves plots & logs under outputs_model2B/.
# ===============================================================
...
class u_Config:
    ...
    u_output_root: str = r"C:\Users\Udhaya\NN_LAB_PRJ_PHASE2\outputs_model2B"
    ...
    # === Training ===
    u_epochs: int = 5                      # fixed for ablations
    u_batch_size: int = 16
    u_num_workers: int = 4
    u_lr: float = 1e-4
...
class u_MMDataset(Dataset):
    """
    We ignore the path stored in CSV and always load images by basename
    from cfg.u_images_dir to keep things consistent.
    """
...
class u_TextEnc(nn.Module):
    def __init__(self, dim_in: int, dim_h: int = 256, p: float = 0.3):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(dim_in, 512), nn.ReLU(inplace=True), nn.Dropout(p),
            nn.Linear(512, dim_h), nn.ReLU(inplace=True), nn.Dropout(p)
        )
    def forward(self, x):
        return self.net(x)
...
def u_plot_loss(u_hist: Dict[str, List[float]], out_png: str):
    ...
    plt.xlabel("Epoch"); plt.ylabel("Loss"); plt.title("Model-2B Training Loss")
...
def u_plot_roc_pr(y_true: np.ndarray, y_prob: np.ndarray, labels: List[str], out_dir: str):
    ...
    plt.xlabel("FPR"); plt.ylabel("TPR"); plt.title("Model-2B: Val ROC")
    plt.legend(fontsize=8); plt.tight_layout()
    plt.savefig(os.path.join(out_dir,"u2B_val_roc.png"), dpi=220); plt.close()
    # PR
    plt.figure(figsize=(9,7))
    for i, lbl in enumerate(labels):
        try:
    ...
    plt.xlabel("Recall"); plt.ylabel("Precision"); plt.title("Model-2B: Val PR")
    plt.legend(fontsize=8); plt.tight_layout()
    plt.savefig(os.path.join(out_dir,"u2B_val_pr.png"), dpi=220); plt.close()
...
def u_predict(model: nn.Module, loader: DataLoader, dev: torch.device):
    """Forward pass on a loader -> (probs, targets, names)."""
    model.eval()
    probs, targets, names = [], [], []
    for img, txt, y, n in loader:
        ...
        names.extend(list(n))
    return np.vstack(probs), np.vstack(targets), names


def u_eval_metrics(y_true: np.ndarray, y_prob: np.ndarray, labels: List[str]) -> Dict[str, object]:
    """Compute AUROC/AUPRC per class and macro averages."""
    roc_s, pr_s = {}, {}
    for i, lbl in enumerate(labels):
        try:
            roc_s[lbl] = roc_auc_score(y_true[:, i], y_prob[:, i])
        except ValueError:
            roc_s[lbl] = float('nan')
        try:
            pr_s[lbl]  = average_precision_score(y_true[:, i], y_prob[:, i])
        except ValueError:
            pr_s[lbl] = float('nan')
    return {
...
def u_main():
    ...
    text_in = vec.transform([""]).shape[1]  # actual dim after fit
    ...
    # history for plots/logs
    hist: Dict[str, List[float]] = {"train_loss": [], "val_macro_auroc": [], "val_macro_auprc": []}

    print("[INFO] Training Model-2B (Augmentation + Dropout)...")
    ...
    # save model
    best_path = os.path.join(cfg.u_models_dir, "u_model2B_best.pt")
    torch.save({
    ...
    # --- plots + logs ---
    with open(os.path.join(logs, "u2B_train_history.json"), "w") as f:
        json.dump(hist, f, indent=2)

    u_plot_loss(hist, os.path.join(plots, "u2B_loss_curves.png"))
    u_plot_metric(hist, "val_macro_auroc", "Validation Macro AUROC (2B)", os.path.join(plots, "u2B_val_macro_auroc.png"))
    u_plot_metric(hist, "val_macro_auprc", "Validation Macro AUPRC (2B)", os.path.join(plots, "u2B_val_macro_auprc.png"))

    # Val ROC/PR curves
    val_prob, val_true, _ = u_predict(model, dl_va, dev)
    u_plot_roc_pr(val_true, val_prob, list(labels), plots)

    # Test evaluation @ 0.5 threshold (for reports & confusion)
    ...
    with open(os.path.join(logs, "u2B_test_metrics.json"), "w") as f:
        json.dump(u_test_metrics, f, indent=2)
    with open(os.path.join(logs, "u2B_test_classification_report.json"), "w") as f:
        json.dump(u_cls_report, f, indent=2)

    u_plot_confusion(test_true, test_pred, list(labels), os.path.join(plots, "u2B_test_confusion.png"))

    print("\n[DONE] Model-2B finished successfully ")
